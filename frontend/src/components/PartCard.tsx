  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <h3 className="text-base font-semibold text-slate-900 dark:text-slate-100">{part.name}</h3>
        <StatusBadge status={lowStock ? 'LOW STOCK' : 'IN STOCK'} />
      </div>

      {part.description && (
        <p className="mt-2 line-clamp-2 text-sm text-slate-500 dark:text-slate-400">{part.description}</p>
      )}
      ...
      <div className="mt-3 flex flex-wrap gap-1.5">
        ...
        {part.compatibleVehicleModels.slice(0, 2).map((m) => (
          <span key={m} className="rounded-full bg-slate-700/60 px-2.5 py-0.5 text-xs text-slate-700 dark:text-slate-300">
            {m}
          </span>
        ))}
      </div>
      ...
      <div className="mt-auto flex items-center justify-between border-t border-slate-700/50 pt-3">
        <span className="text-lg font-bold text-emerald-600 dark:text-accent-400">{formatCurrency(part.price)}</span>
        <span className={`text-xs font-medium ${lowStock ? 'text-red-700 dark:text-red-400' : 'text-slate-500 dark:text-slate-400'}`}>
